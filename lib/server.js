...
initServer = function() { 
	...
	if( configFilename =findConfigFilename()){
		global._server.serverconf = JSON.parse(fs.readFileSync(configFilename), "utf8");
	}
	else{
	...
	if( !global._server.hasOwnProperty("server_rooms") )
		global._server.server_rooms = [];
...
function addRoom(room){
	ret = true;
	_server.server_rooms.push(room);
