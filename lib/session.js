...
callbacks["f_hello_ok"] = function(session, msg){
	...
	setTimeout(function(){
			session.socket.write(message.newMessage("HELLO"));
		}, 5000);
...
callbacks["f_connect"] = function(session, msg){
	...
	if( session.state == "CONNECT"){
		session.state = "LOGIN";
		session.socket.write(message.newMessage("CONNECT_OK"));
...
callbacks["f_login"] = function(session, msg){
	...
	clearTimeout(session.timer);
	if( msg.username == "gdy"
		&& msg.password =="gdypassword" ){
		session.socket.write(message.newMessage("LOGIN_OK"));
...
function defaultProcessMessage(session, msg){
	if( session.state != "LOGINED" )
		session.socket.end(message.newMessage("ERROR"));
}
