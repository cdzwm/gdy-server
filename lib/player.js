...
module.exports.newSession = function(s){
	...
	s.on("close", onClose);
	s.on("error", onError);
	module.exports.on("msg", onProcessMessage);

	setTimeout(function(){ // check hello message.
	}, 2000);
}
...
function onProcessMessage(session){
	if(session.mq.length > 0){
		var msg = session.mq.shift();
		console.log("Process message: " + msg.cmd);
		session.socket.write(msg.cmd + " is OK.");
	}
}